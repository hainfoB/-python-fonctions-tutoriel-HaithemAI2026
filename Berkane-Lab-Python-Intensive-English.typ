...
*Memory trace.*
1. #text("prenom = \"Lina\"\nprint(\"Bienvenue\", prenom)") → memory observes the active instruction and updates the symbolic state.
...
*Memory trace.*
1. #text("age = int(\"16\")\nprint(age + 2)") → memory observes the active instruction and updates the symbolic state.
...
*Memory trace.*
1. #text("a = float(\"12.5\")\nb = float(\"7.5\")\nprint(a + b)") → memory observes the active instruction and updates the symbolic state.
...
*Memory trace.*
1. #text("valeur = 12\nprint(type(valeur).__name__)") → memory observes the active instruction and updates the symbolic state.
...
*Memory trace.*
1. #text("texte = \"42\"\nnombre = int(texte)\nprint(nombre, nombre * 2)") → memory observes the active instruction and updates the symbolic state.
...
*Memory trace.*
1. #text("distance = 42\nduree = 2\nprint(distance / duree)") → memory observes the active instruction and updates the symbolic state.
...
*Memory trace.*
1. #text("celsius = 25\nkelvin = celsius + 273.15\nprint(kelvin)") → memory observes the active instruction and updates the symbolic state.
...
*Memory trace.*
1. #text("masse = 2\ng = 9.81\nhauteur = 3\nprint(masse * g * hauteur)") → memory observes the active instruction and updates the symbolic state.
...
*Memory trace.*
1. #text("age = 16\nfcm = 220 - age\nprint(fcm)") → memory observes the active instruction and updates the symbolic state.
...
*Memory trace.*
1. #text("tension = 12\nresistance = 6\nintensite = tension / resistance\nprint(intensite)") → memory observes the active instruction and updates the symbolic state.
...
*Memory trace.*
1. #text("ville = input(\"Ville : \")\nprint(\"Bienvenue à\", ville)") → memory observes the active instruction and updates the symbolic state.
...
*Memory trace.*
1. #text("age = int(input(\"Age : \"))\nprint(age + 1)") → memory observes the active instruction and updates the symbolic state.
...
*Memory trace.*
1. #text("c = 20\nprint(c * 9 / 5 + 32)") → memory observes the active instruction and updates the symbolic state.
...
*Memory trace.*
1. #text("longueur, largeur = 5, 3\nprint(longueur * largeur)") → memory observes the active instruction and updates the symbolic state.
...
*Memory trace.*
1. #text("print(17 % 5)") → memory observes the active instruction and updates the symbolic state.
...
*Memory trace.*
1. #text("print(\"Lina\" + \" · Oran\")") → memory observes the active instruction and updates the symbolic state.
...
*Memory trace.*
1. #text("print(round(12.567, 2))") → memory observes the active instruction and updates the symbolic state.
...
*Memory trace.*
1. #text("print(12 > 0)") → memory observes the active instruction and updates the symbolic state.
...
*Memory trace.*
1. #text("a, b = 2, 5\na, b = b, a") → memory observes the active instruction and updates the symbolic state.
...
*Memory trace.*
1. #text("score = 15\nprint(\"Score :\", score, \"/ 20\")") → memory observes the active instruction and updates the symbolic state.
...
*Memory trace.*
1. #text("volume = 10\nduree = 5\nprint(volume / duree)") → memory observes the active instruction and updates the symbolic state.
...
*Memory trace.*
1. #text("masse = 250\nvolume = 100\nprint(masse / volume)") → memory observes the active instruction and updates the symbolic state.
...
*Memory trace.*
1. #text("kcal_minute = 8\nduree = 30\nprint(kcal_minute * duree)") → memory observes the active instruction and updates the symbolic state.
...
*Memory trace.*
1. #text("adn = \"ATGCTT\"\nprint(adn.replace(\"T\", \"U\"))") → memory observes the active instruction and updates the symbolic state.
...
*Memory trace.*
1. #text("from math import pi, sqrt\nlongueur = 1\ng = 9.81\nprint(2 * pi * sqrt(longueur / g))") → memory observes the active instruction and updates the symbolic state.
...
*Memory trace.*
1. #text("n = 18\nif n % 2 == 0: print(\"pair\")\nelse: print(\"impair\")") → memory observes the active instruction and updates the symbolic state.
...
*Memory trace.*
1. #text("score = 14\nif score >= 10: print(\"réussi\")\nelse: print(\"à revoir\")") → memory observes the active instruction and updates the symbolic state.
...
*Memory trace.*
1. #text("t = 28\nif t < 10: cat = \"froid\"\nelif t < 25: cat = \"doux\"\nelse: cat = \"chaud\"") → memory observes the active instruction and updates the symbolic state.
...
*Memory trace.*
1. #text("compteur = 0\nfor i in range(5): compteur += 1\nprint(compteur)") → memory observes the active instruction and updates the symbolic state.
...
*Memory trace.*
1. #text("total = 0\nfor n in range(1, 7):\n    if n % 2 == 0: total += n\nprint(total)") → memory observes the active instruction and updates the symbolic state.
...
*Memory trace.*
1. #text("t = -4\nif t < 0: print(\"solide\")\nelif t < 100: print(\"liquide\")\nelse: print(\"gaz\")") → memory observes the active instruction and updates the symbolic state.
...
*Memory trace.*
1. #text("imc = 27.4\nif imc < 18.5: c = \"insuffisant\"\nelif imc < 25: c = \"normal\"\nelif imc < 30: c = \"surpoids\"\nelse: c = \"obésité\"\nprint(c)") → memory observes the active instruction and updates the symbolic state.
...
*Memory trace.*
1. #text("a, b, c = 3, 3, 5\nif a == b == c: print(\"équilatéral\")\nelif a == b or a == c or b == c: print(\"isocèle\")\nelse: print(\"scalène\")") → memory observes the active instruction and updates the symbolic state.
...
*Memory trace.*
1. #text("ph = 8.2\nif ph < 7: print(\"acide\")\nelif ph == 7: print(\"neutre\")\nelse: print(\"basique\")") → memory observes the active instruction and updates the symbolic state.
...
*Memory trace.*
1. #text("uv = 7\nif uv <= 2: conseil = \"faible\"\nelif uv <= 5: conseil = \"modérée\"\nelse: conseil = \"forte\"\nprint(conseil)") → memory observes the active instruction and updates the symbolic state.
...
*Memory trace.*
1. #text("n = -2\nprint(n > 0)") → memory observes the active instruction and updates the symbolic state.
...
*Memory trace.*
1. #text("note = 11\nprint(\"admis\" if note >= 10 else \"refusé\")") → memory observes the active instruction and updates the symbolic state.
...
*Memory trace.*
1. #text("n = 12\nprint(n % 3 == 0)") → memory observes the active instruction and updates the symbolic state.
...
*Memory trace.*
1. #text("v = 65\nif v < 50: r=\"lente\"\nelif v <= 90: r=\"normale\"\nelse: r=\"rapide\"") → memory observes the active instruction and updates the symbolic state.
...
*Memory trace.*
1. #text("total = 0\nfor n in range(1, 6): total += n") → memory observes the active instruction and updates the symbolic state.
...
*Memory trace.*
1. #text("for _ in range(3): print(\"go\")") → memory observes the active instruction and updates the symbolic state.
...
*Memory trace.*
1. #text("i = 1\nwhile i <= 3:\n    print(i); i += 1") → memory observes the active instruction and updates the symbolic state.
...
*Memory trace.*
1. #text("compte = 0\nfor n in [8, 12, 15]:\n    if n > 10: compte += 1") → memory observes the active instruction and updates the symbolic state.
...
*Memory trace.*
1. #text("maximum = 0\nfor n in [4, 9, 2]:\n    if n > maximum: maximum = n") → memory observes the active instruction and updates the symbolic state.
...
*Memory trace.*
1. #text("for n in [1, 4, 7]:\n    if n == 4: break") → memory observes the active instruction and updates the symbolic state.
...
*Memory trace.*
1. #text("densite = 0.8\nprint(\"flotte\" if densite < 1 else \"coule\")") → memory observes the active instruction and updates the symbolic state.
...
*Memory trace.*
1. #text("annee = 2024\nprint(annee % 400 == 0 or (annee % 4 == 0 and annee % 100 != 0))") → memory observes the active instruction and updates the symbolic state.
...
*Memory trace.*
1. #text("vergence = -2\nprint(\"convergente\" if vergence > 0 else \"divergente\")") → memory observes the active instruction and updates the symbolic state.
...
*Memory trace.*
1. #text("systolique = 145\nif systolique > 140: print(\"à surveiller\")\nelse: print(\"dans la zone\")") → memory observes the active instruction and updates the symbolic state.
...
*Memory trace.*
1. #text("a, b, c = 3, 4, 5\nprint(a*a + b*b == c*c)") → memory observes the active instruction and updates the symbolic state.
...
*Memory trace.*
1. #text("temperatures = [18, 22, 19, 25]\nmaximum = temperatures[0]\nfor t in temperatures:\n    if t > maximum: maximum = t\nprint(maximum)") → memory observes the active instruction and updates the symbolic state.
...
*Memory trace.*
1. #text("notes = [8, 12, 15, 9]\nreussites = []\nfor note in notes:\n    if note >= 10: reussites.append(note)\nprint(reussites)") → memory observes the active instruction and updates the symbolic state.
...
*Memory trace.*
1. #text("etapes = [\"A\", \"B\", \"C\"]\nprint(etapes[::-1])") → memory observes the active instruction and updates the symbolic state.
...
*Memory trace.*
1. #text("notes = []\nif notes:\n    print(sum(notes) / len(notes))\nelse:\n    print(\"Aucune note\")") → memory observes the active instruction and updates the symbolic state.
...
*Memory trace.*
1. #text("distances = [12, 18, 15, 21]\nfor d in distances: print(d / 2)") → memory observes the active instruction and updates the symbolic state.
...
*Memory trace.*
1. #text("mesures = [18, 22, 25, 19]\nchaudes = [t for t in mesures if t > 20]\nprint(chaudes)") → memory observes the active instruction and updates the symbolic state.
...
*Memory trace.*
1. #text("energies = [3.2, 4.1, 2.7]\nprint(sum(energies))") → memory observes the active instruction and updates the symbolic state.
...
*Memory trace.*
1. #text("temperatures = [19, 24, 22, 27]\nprint(temperatures.index(max(temperatures)))") → memory observes the active instruction and updates the symbolic state.
...
*Memory trace.*
1. #text("mesures = [10, 20, 30]\nmini, maxi = min(mesures), max(mesures)\nprint([(x-mini)/(maxi-mini) for x in mesures])") → memory observes the active instruction and updates the symbolic state.
...
*Memory trace.*
1. #text("valeurs = [2, 5, 8]\nprint(sum(n % 2 == 0 for n in valeurs))") → memory observes the active instruction and updates the symbolic state.
...
*Memory trace.*
1. #text("valeurs = [7, 3, 9]\nminimum = valeurs[0]\nfor v in valeurs:\n    if v < minimum: minimum = v\nprint(minimum)") → memory observes the active instruction and updates the symbolic state.
...
*Memory trace.*
1. #text("for i in range(len(valeurs)):\n    print(i, valeurs[i])") → memory observes the active instruction and updates the symbolic state.
...
*Memory trace.*
1. #text("valeurs = [1, 4, 7]\nmodifiees = [v + 2 for v in valeurs]") → memory observes the active instruction and updates the symbolic state.
...
*Memory trace.*
1. #text("sortie = []\nfor v in [2, 2, 3, 2]:\n    if v not in sortie: sortie.append(v)") → memory observes the active instruction and updates the symbolic state.
...
*Memory trace.*
1. #text("valeurs = [\"A\", \"B\", \"C\"]\nvaleurs = valeurs[1:] + valeurs[:1]") → memory observes the active instruction and updates the symbolic state.
...
*Memory trace.*
1. #text("total = 0\nfor v in [-2, 4, -1, 6]:\n    if v > 0: total += v") → memory observes the active instruction and updates the symbolic state.
...
*Memory trace.*
1. #text("valeurs = [4, 9, 2]\nfor i, v in enumerate(valeurs):\n    if v == 9: print(i); break") → memory observes the active instruction and updates the symbolic state.
...
*Memory trace.*
1. #text("temperatures = [23, 18, 25]\nprint(sorted(temperatures))") → memory observes the active instruction and updates the symbolic state.
...
*Memory trace.*
1. #text("valeurs = [4, 10, 7]\nprint(max(valeurs) - min(valeurs))") → memory observes the active instruction and updates the symbolic state.
...
*Memory trace.*
1. #text("vitesses = [8, 10, 12, 10]\nprint(sum(vitesses) / len(vitesses))") → memory observes the active instruction and updates the symbolic state.
...
*Memory trace.*
1. #text("temperatures = [-2, 4, -1, 3]\nprint(sum(t < 0 for t in temperatures))") → memory observes the active instruction and updates the symbolic state.
...
*Memory trace.*
1. #text("distances = [2, 3, 4]\ncumul = 0\nfor d in distances:\n    cumul += d\nprint(cumul)") → memory observes the active instruction and updates the symbolic state.
...
*Memory trace.*
1. #text("courants = [2.1, 3.4, 2.8]\nprint(courants.index(max(courants)))") → memory observes the active instruction and updates the symbolic state.
...
*Memory trace.*
1. #text("longueurs = [30, 45, 120]\nprint([x / 100 for x in longueurs])") → memory observes the active instruction and updates the symbolic state.
...
*Memory trace.*
1. #text("profil = {\"nom\": \"Lina\", \"ville\": \"Oran\", \"age\": 16}\nprint(profil[\"ville\"])") → memory observes the active instruction and updates the symbolic state.
...
*Memory trace.*
1. #text("stock = {\"clavier\": 8, \"souris\": 12}\nstock[\"clavier\"] -= 1") → memory observes the active instruction and updates the symbolic state.
...
*Memory trace.*
1. #text("contact = {\"nom\": \"Ali\"}\nprint(contact.get(\"telephone\", \"Absent\"))") → memory observes the active instruction and updates the symbolic state.
...
*Memory trace.*
1. #text("notes = {\"maths\": 15, \"python\": 17}\nfor matiere, note in notes.items():\n    print(matiere, note)") → memory observes the active instruction and updates the symbolic state.
...
*Memory trace.*
1. #text("stock = {\"livres\": 4, \"cahiers\": 9}\nprint(sum(stock.values()))") → memory observes the active instruction and updates the symbolic state.
...
*Memory trace.*
1. #text("mesure = {\"valeur\": 21.4, \"unite\": \"°C\", \"capteur\": \"S1\"}\nprint(mesure[\"valeur\"], mesure[\"unite\"])") → memory observes the active instruction and updates the symbolic state.
...
*Memory trace.*
1. #text("fc = {\"repos\": 65, \"course\": 154}\nprint(fc.get(\"natation\", \"non mesurée\"))") → memory observes the active instruction and updates the symbolic state.
...
*Memory trace.*
1. #text("proportions = {\"eau\": 70, \"sel\": 5, \"solvant\": 25}\nprint(sum(proportions.values()))") → memory observes the active instruction and updates the symbolic state.
...
*Memory trace.*
1. #text("compte = {}\nfor etat in [\"solide\", \"liquide\", \"liquide\"]:\n    compte[etat] = compte.get(etat, 0) + 1\nprint(compte)") → memory observes the active instruction and updates the symbolic state.
...
*Memory trace.*
1. #text("protocole = {\"duree\": 60, \"temperature\": 25, \"agitation\": True}\nfor cle, valeur in protocole.items(): print(cle, valeur)") → memory observes the active instruction and updates the symbolic state.
...
*Memory trace.*
1. #text("profil = {\"nom\": \"Sara\"}\nprofil[\"email\"] = \"sara@mail.test\"") → memory observes the active instruction and updates the symbolic state.
...
*Memory trace.*
1. #text("donnees = {\"nom\": \"Ali\", \"temp\": True}\ndel donnees[\"temp\"]") → memory observes the active instruction and updates the symbolic state.
...
*Memory trace.*
1. #text("votes = {}\nfor nom in [\"A\", \"B\", \"A\"]:\n    votes[nom] = votes.get(nom, 0) + 1") → memory observes the active instruction and updates the symbolic state.
...
*Memory trace.*
1. #text("stock = {\"A\": 2, \"B\": 8}\nfor nom, quantite in stock.items():\n    if quantite < 5: print(nom)") → memory observes the active instruction and updates the symbolic state.
...
*Memory trace.*
1. #text("source = {\"nom\": \"Lina\", \"age\": 16, \"secret\": 1}\ncible = {k: source[k] for k in [\"nom\", \"age\"]}") → memory observes the active instruction and updates the symbolic state.
...
*Memory trace.*
1. #text("codes = {\"Oran\": 31, \"Alger\": 16}\ninverse = {code: ville for ville, code in codes.items()}") → memory observes the active instruction and updates the symbolic state.
...
*Memory trace.*
1. #text("notes = {\"A\": 12, \"B\": 16}\nprint(sum(notes.values()) / len(notes))") → memory observes the active instruction and updates the symbolic state.
...
*Memory trace.*
1. #text("profil = {\"nom\": \"Maya\"}\nprint(profil.get(\"ville\", \"inconnue\"))") → memory observes the active instruction and updates the symbolic state.
...
*Memory trace.*
1. #text("classes = {}\nfor nom, classe in [(\"A\", \"2A\"), (\"B\", \"2A\")]:\n    classes.setdefault(classe, []).append(nom)") → memory observes the active instruction and updates the symbolic state.
...
*Memory trace.*
1. #text("fiche = {\"id\": \"S1\", \"nom\": \"Lina\"}\nfor cle, valeur in fiche.items(): print(cle, \":\", valeur)") → memory observes the active instruction and updates the symbolic state.
...
*Memory trace.*
1. #text("scores = {\"Lina\": 16, \"Ali\": 12}\nprint(scores[\"Lina\"])") → memory observes the active instruction and updates the symbolic state.
...
*Memory trace.*
1. #text("stock = {\"cahier\": 12}\nif \"cahier\" in stock: stock[\"cahier\"] -= 1") → memory observes the active instruction and updates the symbolic state.
...
*Memory trace.*
1. #text("noms = {\"Na\": \"sodium\", \"O\": \"oxygene\"}\ninverse = {nom: code for code, nom in noms.items()}") → memory observes the active instruction and updates the symbolic state.
...
*Memory trace.*
1. #text("mesure = {\"temperature\": 20}\nprint(mesure.get(\"pression\", \"absente\"))") → memory observes the active instruction and updates the symbolic state.
...
*Memory trace.*
1. #text("eleve = {\"id\": \"S1\", \"nom\": \"Maya\", \"notes\": [12, 15]}") → memory observes the active instruction and updates the symbolic state.
...
*Memory trace.*
1. #text("note = 18\nvalide = 0 <= note <= 20\nprint(valide)") → memory observes the active instruction and updates the symbolic state.
...
*Memory trace.*
1. #text("eleves = [{\"nom\":\"A\",\"note\":12},{\"nom\":\"B\",\"note\":8}]\nfor eleve in eleves: print(eleve[\"nom\"])") → memory observes the active instruction and updates the symbolic state.
...
*Memory trace.*
1. #text("eleve = {\"nom\":\"Lina\", \"notes\":[14, 16]}\neleve[\"moyenne\"] = sum(eleve[\"notes\"]) / len(eleve[\"notes\"])") → memory observes the active instruction and updates the symbolic state.
...
*Memory trace.*
1. #text("eleves = [{\"id\":\"S1\"},{\"id\":\"S2\"}]\nfor e in eleves:\n    if e[\"id\"] == \"S2\": print(e)") → memory observes the active instruction and updates the symbolic state.
...
*Memory trace.*
1. #text("athlete = {\"nom\": \"Nora\", \"distance\": 5, \"temps\": 28}\nprint(athlete[\"distance\"] / (athlete[\"temps\"] / 60))") → memory observes the active instruction and updates the symbolic state.
...
*Memory trace.*
1. #text("experience = {\"temperature\": 25, \"masse\": 12.4, \"resultat\": \"stable\"}\nif experience[\"resultat\"] == \"stable\": print(\"valider\")") → memory observes the active instruction and updates the symbolic state.
...
*Memory trace.*
1. #text("dossier = {\"nom\": \"Samir\", \"email\": \"samir@example.com\"}\nobligatoires = [\"nom\", \"email\"]\nprint(all(cle in dossier for cle in obligatoires))") → memory observes the active instruction and updates the symbolic state.
...
*Memory trace.*
1. #text("mesures = [{\"id\": 1, \"valeur\": 18}, {\"id\": 2, \"valeur\": 22}]\nprint([m[\"valeur\"] for m in mesures])") → memory observes the active instruction and updates the symbolic state.
...
*Memory trace.*
1. #text("eleve = {\"prenom\": \"Lina\", \"notes\": [14, 16]}\nmoy = sum(eleve[\"notes\"]) / len(eleve[\"notes\"])\nprint(f\"{eleve[\"prenom\"]}: {moy}\")") → memory observes the active instruction and updates the symbolic state.
...
*Memory trace.*
1. #text("livre = {\"titre\":\"Python\", \"annee\":2026}") → memory observes the active instruction and updates the symbolic state.
...
*Memory trace.*
1. #text("fiche = {\"nom\":\"\"}\nprint(bool(fiche.get(\"nom\")))") → memory observes the active instruction and updates the symbolic state.
...
*Memory trace.*
1. #text("for e in eleves: e[\"moyenne\"] = sum(e[\"notes\"]) / len(e[\"notes\"])") → memory observes the active instruction and updates the symbolic state.
...
*Memory trace.*
1. #text("admis = [e for e in eleves if e[\"moyenne\"] >= 10]") → memory observes the active instruction and updates the symbolic state.
...
*Memory trace.*
1. #text("ids = [e[\"id\"] for e in eleves]\nprint(len(ids) == len(set(ids)))") → memory observes the active instruction and updates the symbolic state.
...
*Memory trace.*
1. #text("capteur = {\"nom\":\"T1\", \"unite\":\"°C\", \"mesure\":21.4}") → memory observes the active instruction and updates the symbolic state.
...
*Memory trace.*
1. #text("m = 21.4\nprint(0 <= m <= 50)") → memory observes the active instruction and updates the symbolic state.
...
*Memory trace.*
1. #text("for e in eleves: print(e.get(\"nom\", \"inconnu\"))") → memory observes the active instruction and updates the symbolic state.
...
*Memory trace.*
1. #text("trouve = next((e for e in eleves if e[\"id\"] == \"S2\"), None)") → memory observes the active instruction and updates the symbolic state.
...
*Memory trace.*
1. #text("valides = [e for e in eleves if e.get(\"moyenne\", 0) >= 10]\nprint(len(valides))") → memory observes the active instruction and updates the symbolic state.
...
*Memory trace.*
1. #text("mesure = {\"systolique\": 120, \"diastolique\": 80, \"date\": \"2026-06-01\"}") → memory observes the active instruction and updates the symbolic state.
...
*Memory trace.*
1. #text("mesures = [{\"valeur\": 18}, {\"valeur\": 24}]\nprint([m for m in mesures if m[\"valeur\"] > 20])") → memory observes the active instruction and updates the symbolic state.
...
*Memory trace.*
1. #text("eleves = [{\"id\": 1}, {\"id\": 2}]\nprint(all(\"id\" in e for e in eleves))") → memory observes the active instruction and updates the symbolic state.
...
*Memory trace.*
1. #text("produit = {\"nom\": \"Tube\", \"stock\": 4}\nproduit[\"stock\"] += 6") → memory observes the active instruction and updates the symbolic state.
...
*Memory trace.*
1. #text("eleves = [{\"id\": 1, \"nom\": \"Ali\"}, {\"id\": 2, \"nom\": \"Nora\"}]\nprint(next(e for e in eleves if e[\"id\"] == 2))") → memory observes the active instruction and updates the symbolic state.
...
*Memory trace.*
1. #text("pile = [\"titre\", \"image\"]\nprint(pile.pop())") → memory observes the active instruction and updates the symbolic state.
...
*Memory trace.*
1. #text("historique = [\"accueil\", \"cours\", \"quiz\"]\npage = historique.pop()\nprint(page, historique[-1])") → memory observes the active instruction and updates the symbolic state.
...
*Memory trace.*
1. #text("pile = []\nfor symbole in \"(()\":\n    if symbole == \"(\": pile.append(symbole)\nprint(len(pile))") → memory observes the active instruction and updates the symbolic state.
...
*Memory trace.*
1. #text("pile = []\nif pile: print(pile.pop())\nelse: print(\"Rien à annuler\")") → memory observes the active instruction and updates the symbolic state.
...
*Memory trace.*
1. #text("actions = [\"A\", \"B\", \"C\"]\nfor _ in range(2): print(actions.pop())") → memory observes the active instruction and updates the symbolic state.
...
*Memory trace.*
1. #text("pile = list(\"ATGC\")\nretour = \"\"\nwhile pile: retour += pile.pop()\nprint(retour)") → memory observes the active instruction and updates the symbolic state.
...
*Memory trace.*
1. #text("historique = [\"filtre\", \"normalisation\", \"arrondi\"]\nprint(\"annuler\", historique.pop())") → memory observes the active instruction and updates the symbolic state.
...
*Memory trace.*
1. #text("appels = [\"main\", \"analyser\", \"moyenne\"]\nprint(appels[-1])\nappels.pop()") → memory observes the active instruction and updates the symbolic state.
...
*Memory trace.*
1. #text("pile = []\nfor action in [\"A\", \"B\", \"C\"]: pile.append(action)") → memory observes the active instruction and updates the symbolic state.
...
*Memory trace.*
1. #text("if pile: resultat = pile.pop()") → memory observes the active instruction and updates the symbolic state.
...
*Memory trace.*
1. #text("sommet = pile[-1] if pile else None") → memory observes the active instruction and updates the symbolic state.
...
*Memory trace.*
1. #text("print(len(pile))") → memory observes the active instruction and updates the symbolic state.
...
*Memory trace.*
1. #text("while pile: pile.pop()") → memory observes the active instruction and updates the symbolic state.
...
*Memory trace.*
1. #text("pile = list(\"PY\")\nprint(pile.pop() + pile.pop())") → memory observes the active instruction and updates the symbolic state.
...
*Memory trace.*
1. #text("pile = []\nfor x in [1,2]:\n    pile.append(x); print(pile)") → memory observes the active instruction and updates the symbolic state.
...
*Memory trace.*
1. #text("page = historique[-1] if historique else \"aucune\"") → memory observes the active instruction and updates the symbolic state.
...
*Memory trace.*
1. #text("if len(pile) < 3: pile.append(\"nouveau\")") → memory observes the active instruction and updates the symbolic state.
...
*Memory trace.*
1. #text("def depiler(pile):\n    return pile.pop() if pile else None") → memory observes the active instruction and updates the symbolic state.
...
*Memory trace.*
1. #text("mot = \"kayak\"\npile = list(mot)\nprint(mot == \"\".join(pile.pop() for _ in range(len(pile))))") → memory observes the active instruction and updates the symbolic state.
...
*Memory trace.*
1. #text("pile = [10, 20, 30]\nprint(pile[-1])") → memory observes the active instruction and updates the symbolic state.
...
*Memory trace.*
1. #text("pile = [1, 2, 3]\nwhile pile: print(pile.pop())") → memory observes the active instruction and updates the symbolic state.
...
*Memory trace.*
1. #text("file = [\"A.pdf\", \"B.pdf\"]\nprint(file.pop(0))") → memory observes the active instruction and updates the symbolic state.
...
*Memory trace.*
1. #text("file = [\"Nora\", \"Ali\", \"Maya\"]\nservie = file.pop(0)") → memory observes the active instruction and updates the symbolic state.
...
*Memory trace.*
1. #text("file = []\nif file: print(file.pop(0))\nelse: print(\"Attente vide\")") → memory observes the active instruction and updates the symbolic state.
...
*Memory trace.*
1. #text("file = [\"A\", \"B\"]\nfile.append(\"C\")\nprint(file)") → memory observes the active instruction and updates the symbolic state.
...
*Memory trace.*
1. #text("messages = [\"T=20\", \"T=21\"]\nwhile messages:\n    print(\"analyse\", messages.pop(0))") → memory observes the active instruction and updates the symbolic state.
...
*Memory trace.*
1. #text("demandes = [\"A\", \"B\", \"C\"]\nprochaine = demandes.pop(0)\nprint(prochaine)") → memory observes the active instruction and updates the symbolic state.
...
*Memory trace.*
1. #text("documents = []\nfor nom in [\"a.pdf\", \"b.pdf\"]: documents.append(nom)\nprint(documents.pop(0))") → memory observes the active instruction and updates the symbolic state.
...
*Memory trace.*
1. #text("file = []\nfile.append(\"T1\"); file.append(\"T2\")") → memory observes the active instruction and updates the symbolic state.
...
*Memory trace.*
1. #text("ticket = file.pop(0) if file else None") → memory observes the active instruction and updates the symbolic state.
...
*Memory trace.*
1. #text("print(len(file))") → memory observes the active instruction and updates the symbolic state.
...
*Memory trace.*
1. #text("file.insert(0, \"URGENT\")") → memory observes the active instruction and updates the symbolic state.
...
*Memory trace.*
1. #text("servis = 0\nwhile file:\n    file.pop(0); servis += 1") → memory observes the active instruction and updates the symbolic state.
...
*Memory trace.*
1. #text("def servir(file):\n    return file.pop(0) if file else None") → memory observes the active instruction and updates the symbolic state.
...
*Memory trace.*
1. #text("file.append(file.pop(0))") → memory observes the active instruction and updates the symbolic state.
...
*Memory trace.*
1. #text("temps = [4, 7, 5]\nprint(sum(temps) / len(temps))") → memory observes the active instruction and updates the symbolic state.
...
*Memory trace.*
1. #text("file1 = [\"A\",\"B\"]; file2 = [\"C\",\"D\"]\nfile = file1 + file2") → memory observes the active instruction and updates the symbolic state.
...
*Memory trace.*
1. #text("file = [\"T1\"]\nfile.append(\"T2\")") → memory observes the active instruction and updates the symbolic state.
...
*Memory trace.*
1. #text("file = [1, 2, 3, 4, 5]\nwhile file: print(file.pop(0))") → memory observes the active instruction and updates the symbolic state.
...
*Memory trace.*
1. #text("file = [\"A\", \"B\", \"C\"]\nfile.pop(0)\nprint(len(file))") → memory observes the active instruction and updates the symbolic state.
...
*Memory trace.*
1. #text("arbre = {\"valeur\":\"Projet\", \"enfants\":[]}\nprint(arbre[\"valeur\"])") → memory observes the active instruction and updates the symbolic state.
...
*Memory trace.*
1. #text("noeud = {\"valeur\":\"README\", \"enfants\":[]}\nprint(noeud[\"enfants\"] == [])") → memory observes the active instruction and updates the symbolic state.
...
*Memory trace.*
1. #text("def visiter(n):\n    print(n[\"valeur\"])\n    for enfant in n[\"enfants\"]: visiter(enfant)") → memory observes the active instruction and updates the symbolic state.
...
*Memory trace.*
1. #text("def hauteur(n):\n    if not n[\"enfants\"]: return 1\n    return 1 + max(hauteur(e) for e in n[\"enfants\"])") → memory observes the active instruction and updates the symbolic state.
...
*Memory trace.*
1. #text("def contient(n, cible):\n    return n[\"valeur\"] == cible or any(contient(e, cible) for e in n[\"enfants\"])") → memory observes the active instruction and updates the symbolic state.
...
*Memory trace.*
1. #text("racine = {\"nom\":\"cours\", \"enfants\":[{\"nom\":\"python\", \"enfants\":[]}, {\"nom\":\"data\", \"enfants\":[]}] }\nprint(len(racine[\"enfants\"]))") → memory observes the active instruction and updates the symbolic state.
...
*Memory trace.*
1. #text("def decision(note):\n    return {\"valeur\":\"reussi\" if note >= 10 else \"a_reprendre\", \"enfants\":[]}\nprint(decision(14))") → memory observes the active instruction and updates the symbolic state.
...
*Memory trace.*
1. #text("def feuilles(n):\n    if not n[\"enfants\"]: return 1\n    return sum(feuilles(e) for e in n[\"enfants\"])") → memory observes the active instruction and updates the symbolic state.
...
*Memory trace.*
1. #text("def afficher(n, niveau=0):\n    print(\"  \" * niveau + n[\"valeur\"])\n    for e in n[\"enfants\"]: afficher(e, niveau + 1)") → memory observes the active instruction and updates the symbolic state.
...
*Memory trace.*
1. #text("arbre = {\"valeur\":\"A\", \"enfants\":[{\"valeur\":\"B\",\"enfants\":[]},{\"valeur\":\"C\",\"enfants\":[]}]}") → memory observes the active instruction and updates the symbolic state.
...
*Memory trace.*
1. #text("def afficher(n):\n    print(n[\"valeur\"])\n    for e in n[\"enfants\"]: afficher(e)") → memory observes the active instruction and updates the symbolic state.
...
*Memory trace.*
1. #text("def compter(n):\n    return 1 + sum(compter(e) for e in n[\"enfants\"])") → memory observes the active instruction and updates the symbolic state.
...
*Memory trace.*
1. #text("def feuille(n): return len(n[\"enfants\"]) == 0") → memory observes the active instruction and updates the symbolic state.
...
*Memory trace.*
1. #text("def h(n):\n    return 1 if not n[\"enfants\"] else 1 + max(h(e) for e in n[\"enfants\"])") → memory observes the active instruction and updates the symbolic state.
...
*Memory trace.*
1. #text("trouve = contient(arbre, \"Cours\")") → memory observes the active instruction and updates the symbolic state.
...
*Memory trace.*
1. #text("def afficher(n, niveau=0):\n    print(\"  \" * niveau + n[\"valeur\"])") → memory observes the active instruction and updates the symbolic state.
...
*Memory trace.*
1. #text("def ok(n): return n[\"valeur\"].startswith(\"C\")") → memory observes the active instruction and updates the symbolic state.
...
*Memory trace.*
1. #text("def afficher(n):\n    if n is None: return\n    print(n[\"valeur\"])") → memory observes the active instruction and updates the symbolic state.
...
*Memory trace.*
1. #text("noeud = {\"valeur\": \"racine\", \"enfants\": []}") → memory observes the active instruction and updates the symbolic state.
...
*Memory trace.*
1. #text("def afficher_feuilles(n):\n    if not n[\"enfants\"]: print(n[\"valeur\"])\n    for e in n[\"enfants\"]: afficher_feuilles(e)") → memory observes the active instruction and updates the symbolic state.
...
*Memory trace.*
1. #text("notes = [8, 12, 14, 15, 19]\nprint(sum(notes) / len(notes))") → memory observes the active instruction and updates the symbolic state.
...
*Memory trace.*
1. #text("temps = [18, 25, 20, 22, 35]\ntries = sorted(temps)\nprint(tries[len(tries)//2])") → memory observes the active instruction and updates the symbolic state.
...
*Memory trace.*
1. #text("mesures = [4, 8, 10]\nprint(max(mesures) - min(mesures))") → memory observes the active instruction and updates the symbolic state.
...
*Memory trace.*
1. #text("mesures = [12.5, 9.8, 15.2]\nprint(min(mesures), max(mesures))") → memory observes the active instruction and updates the symbolic state.
...
*Memory trace.*
1. #text("fc = [62, 70, 78, 66]\nprint(sum(fc)/len(fc), max(fc)-min(fc))") → memory observes the active instruction and updates the symbolic state.
...
*Memory trace.*
1. #text("pop = [100, 120, 150, 190]\nvariations = [pop[i+1]-pop[i] for i in range(len(pop)-1)]\nprint(variations)") → memory observes the active instruction and updates the symbolic state.
...
*Memory trace.*
1. #text("valeurs = sorted([2, 4, 5, 8, 10, 12])\nprint(valeurs[:3], valeurs[3:])") → memory observes the active instruction and updates the symbolic state.
...
*Memory trace.*
1. #text("notes = [10, 12, 10, 15, 12, 10]\nprint({n: notes.count(n) for n in set(notes)})") → memory observes the active instruction and updates the symbolic state.
...
*Memory trace.*
1. #text("moyenne = 13.4\nobjectif = 12\nprint(\"objectif atteint\" if moyenne >= objectif else \"notion à retravailler\")") → memory observes the active instruction and updates the symbolic state.
...
*Memory trace.*
1. #text("valeurs = sorted([2, 8, 4, 6])\nmediane = (valeurs[1] + valeurs[2]) / 2") → memory observes the active instruction and updates the symbolic state.
...
*Memory trace.*
1. #text("valeurs = [4, None, 6]\npropres = [v for v in valeurs if v is not None]") → memory observes the active instruction and updates the symbolic state.
...
*Memory trace.*
1. #text("donnees = [2, 4, 6, 8]\nprint(len(donnees))") → memory observes the active instruction and updates the symbolic state.
...
*Memory trace.*
1. #text("m = sum(notes)/len(notes)\nprint([n for n in notes if n > m])") → memory observes the active instruction and updates the symbolic state.
...
*Memory trace.*
1. #text("a = [10, 12]; b = [14, 16]\nprint(sum(a)/len(a) < sum(b)/len(b))") → memory observes the active instruction and updates the symbolic state.
...
*Memory trace.*
1. #text("notes = [10, 10, 12]\nfrequences = {}\nfor n in notes: frequences[n] = frequences.get(n, 0) + 1") → memory observes the active instruction and updates the symbolic state.
...
*Memory trace.*
1. #text("v = sorted([4, 8, 10, 12, 16])\nprint(sum(v)/len(v), v[len(v)//2], max(v)-min(v))") → memory observes the active instruction and updates the symbolic state.
...
*Memory trace.*
1. #text("m = sum(notes)/len(notes)\nprint(\"niveau global satisfaisant\" if m >= 10 else \"à retravailler\")") → memory observes the active instruction and updates the symbolic state.
...
*Memory trace.*
1. #text("notes = [12, 16]\ncoeffs = [1, 2]\nprint(sum(n*c for n,c in zip(notes, coeffs)) / sum(coeffs))") → memory observes the active instruction and updates the symbolic state.
...
*Memory trace.*
1. #text("ancien, nouveau = 100, 125\nprint((nouveau - ancien) / ancien * 100)") → memory observes the active instruction and updates the symbolic state.
...
*Memory trace.*
1. #text("valeurs = sorted([9, 2, 7, 4, 5])\nprint(valeurs[len(valeurs)//2])") → memory observes the active instruction and updates the symbolic state.
...
*Memory trace.*
1. #text("valeurs = [10, None, 14]\npropres = [v for v in valeurs if v is not None]\nprint(sum(propres)/len(propres))") → memory observes the active instruction and updates the symbolic state.
...
*Memory trace.*
1. #text("a, b = [12, 14], [15, 16]\nma, mb = sum(a)/len(a), sum(b)/len(b)\nprint(\"B\" if mb > ma else \"A\")") → memory observes the active instruction and updates the symbolic state.
...
*Memory trace.*
1. #text("def saluer(nom):\n    return \"Bonjour \" + nom\nprint(saluer(\"Lina\"))") → memory observes the active instruction and updates the symbolic state.
...
*Memory trace.*
1. #text("def carre(n): return n * n\nprint(carre(6))") → memory observes the active instruction and updates the symbolic state.
...
*Memory trace.*
1. #text("def ttc(prix, taux=0.2): return prix * (1+taux)\nprint(ttc(100))") → memory observes the active instruction and updates the symbolic state.
...
*Memory trace.*
1. #text("def valide(note): return 0 <= note <= 20\nprint(valide(18))") → memory observes the active instruction and updates the symbolic state.
...
*Memory trace.*
1. #text("def c_to_f(c): return c * 9 / 5 + 32\ndef chaud(f): return f >= 77\nprint(chaud(c_to_f(25)))") → memory observes the active instruction and updates the symbolic state.
...
*Memory trace.*
1. #text("from math import pi\ndef aire_disque(r):\n    return pi * r ** 2\nprint(aire_disque(3))") → memory observes the active instruction and updates the symbolic state.
...
*Memory trace.*
1. #text("def vitesse(distance_km, temps_h):\n    return distance_km / temps_h\nprint(vitesse(42, 2))") → memory observes the active instruction and updates the symbolic state.
...
*Memory trace.*
1. #text("def energie(masse, g, hauteur):\n    return masse * g * hauteur\nprint(energie(2, 9.81, 3))") → memory observes the active instruction and updates the symbolic state.
...
*Memory trace.*
1. #text("def concentration(masse, volume):\n    return masse / volume\nprint(concentration(12, 0.5))") → memory observes the active instruction and updates the symbolic state.
...
*Memory trace.*
1. #text("def zone_pression(valeur):\n    if valeur < 90: return \"basse\"\n    if valeur <= 120: return \"habituelle\"\n    return \"élevée\"\nprint(zone_pression(128))") → memory observes the active instruction and updates the symbolic state.
...
*Memory trace.*
1. #text("def est_pair(n): return n % 2 == 0") → memory observes the active instruction and updates the symbolic state.
...
*Memory trace.*
1. #text("def moyenne(a,b): return (a+b)/2") → memory observes the active instruction and updates the symbolic state.
...
*Memory trace.*
1. #text("def plus_grand(a,b): return a if a >= b else b") → memory observes the active instruction and updates the symbolic state.
...
*Memory trace.*
1. #text("def remise(prix, taux=10): return prix*(1-taux/100)") → memory observes the active instruction and updates the symbolic state.
...
*Memory trace.*
1. #text("def phrase(nom, ville): return f\"{nom} habite à {ville}.\"") → memory observes the active instruction and updates the symbolic state.
...
*Memory trace.*
1. #text("def secondes(minutes): return minutes*60") → memory observes the active instruction and updates the symbolic state.
...
*Memory trace.*
1. #text("def categorie(age):\n    if age < 18: return \"mineur\"\n    return \"majeur\"") → memory observes the active instruction and updates the symbolic state.
...
*Memory trace.*
1. #text("def total(valeurs): return sum(valeurs)") → memory observes the active instruction and updates the symbolic state.
...
*Memory trace.*
1. #text("assert est_pair(4)\nassert not est_pair(5)") → memory observes the active instruction and updates the symbolic state.
...
*Memory trace.*
1. #text("def carre(n):\n    \"\"\"Renvoie le carré de n.\"\"\"\n    return n*n") → memory observes the active instruction and updates the symbolic state.
...
*Memory trace.*
1. #text("def fcm(age):\n    return 220 - age") → memory observes the active instruction and updates the symbolic state.
...
*Memory trace.*
1. #text("def celsius_vers_kelvin(c):\n    return c + 273.15") → memory observes the active instruction and updates the symbolic state.
...
*Memory trace.*
1. #text("def intensite(u, r):\n    return u / r") → memory observes the active instruction and updates the symbolic state.
...
*Memory trace.*
1. #text("def est_pair(n):\n    return n % 2 == 0") → memory observes the active instruction and updates the symbolic state.
...
*Memory trace.*
1. #text("def vitesse(d, t): return d / t\nassert vitesse(100, 2) == 50\nassert vitesse(30, 1.5) == 20") → memory observes the active instruction and updates the symbolic state.
...
*Memory trace.*
1. #text("t = 18\nif t < 0:\n    etat = \"solide\"\nelif t < 100:\n    etat = \"liquide\"\nelse:\n    etat = \"gaz\"\nprint(etat)") → memory observes the active instruction and updates the symbolic state.
...
*Memory trace.*
1. #text("age = 16\nparent = True\nif age >= 18:\n    autorise = True\nelif age >= 14 and parent:\n    autorise = True\nelse:\n    autorise = False\nprint(autorise)") → memory observes the active instruction and updates the symbolic state.
...
*Memory trace.*
1. #text("note = 15\nif note >= 10:\n    if note >= 16:\n        mention = \"très bien\"\n    else:\n        mention = \"réussite\"\nelse:\n    mention = \"à revoir\"\nprint(mention)") → memory observes the active instruction and updates the symbolic state.
...
*Memory trace.*
1. #text("couleur = \"orange\"\nurgence = False\nif couleur == \"rouge\":\n    action = \"arrêt\"\nelif couleur == \"orange\":\n    if urgence:\n        action = \"prudence renforcée\"\n    else:\n        action = \"ralentir\"\nelse:\n    action = \"avancer\"\nprint(action)") → memory observes the active instruction and updates the symbolic state.
...
*Memory trace.*
1. #text("a, b, c = 3, 4, 5\nif a + b > c and a + c > b and b + c > a:\n    if a*a + b*b == c*c:\n        nature = \"rectangle\"\n    else:\n        nature = \"valide\"\nelse:\n    nature = \"impossible\"\nprint(nature)") → memory observes the active instruction and updates the symbolic state.
...
*Memory trace.*
1. #text("niveau = 22\nchargeur = True\nif niveau < 20:\n    if chargeur:\n        message = \"charger\"\n    else:\n        message = \"chercher une prise\"\nelse:\n    message = \"continuer\"\nprint(message)") → memory observes the active instruction and updates the symbolic state.
...
*Memory trace.*
1. #text("code = \"A7\"\nactif = True\nif actif:\n    if len(code) >= 2:\n        acces = \"autorisé\"\n    else:\n        acces = \"code trop court\"\nelse:\n    acces = \"compte inactif\"\nprint(acces)") → memory observes the active instruction and updates the symbolic state.
...
*Memory trace.*
1. #text("stock = 8\ncommande = 5\nif commande > 0:\n    if commande <= stock:\n        resultat = \"préparer\"\n    else:\n        resultat = \"stock insuffisant\"\nelse:\n    resultat = \"commande invalide\"\nprint(resultat)") → memory observes the active instruction and updates the symbolic state.
...
*Memory trace.*
1. #text("moyenne = 13\npresents = 4\nif presents > 0:\n    if moyenne >= 10:\n        decision = \"validé\"\n    else:\n        decision = \"remédiation\"\nelse:\n    decision = \"aucune décision\"\nprint(decision)") → memory observes the active instruction and updates the symbolic state.
...
*Memory trace.*
1. #text("x = 42\nif x < 10: classe = \"A\"\nelif x < 50: classe = \"B\"\nelse: classe = \"C\"\nprint(classe)") → memory observes the active instruction and updates the symbolic state.
...
*Memory trace.*
1. #text("x = 12\nif 0 <= x <= 20: print(\"ok\")\nelse: print(\"erreur\")") → memory observes the active instruction and updates the symbolic state.
...
*Memory trace.*
1. #text("age = 17\nif age < 6:\n    tarif = 0\nelif age < 18:\n    tarif = 2\nelif age < 65:\n    tarif = 3\nelse:\n    tarif = 1") → memory observes the active instruction and updates the symbolic state.
...
*Memory trace.*
1. #text("score = 74\nif score >= 90:\n    niveau = \"élite\"\nelif score >= 75:\n    niveau = \"avancé\"\nelif score >= 50:\n    niveau = \"intermédiaire\"\nelse:\n    niveau = \"débutant\"\nprint(niveau)") → memory observes the active instruction and updates the symbolic state.
...
*Memory trace.*
1. #text("ressentie = 4\nif ressentie < 0: risque = \"gel\"\nelif ressentie < 8: risque = \"froid\"\nelif ressentie < 18: risque = \"frais\"\nelse: risque = \"doux\"\nprint(risque)") → memory observes the active instruction and updates the symbolic state.
...
*Memory trace.*
1. #text("attente = 48\nif attente > 120: priorite = \"urgente\"\nelif attente > 60: priorite = \"haute\"\nelif attente > 20: priorite = \"normale\"\nelse: priorite = \"basse\"\nprint(priorite)") → memory observes the active instruction and updates the symbolic state.
...
*Memory trace.*
1. #text("imc = 24.2\nif imc < 18.5: categorie = \"bas\"\nelif imc < 25: categorie = \"référence\"\nelif imc < 30: categorie = \"haut\"\nelse: categorie = \"très haut\"\nprint(categorie)") → memory observes the active instruction and updates the symbolic state.
...
*Memory trace.*
1. #text("for i in range(5):\n    print(i)") → memory observes the active instruction and updates the symbolic state.
...
*Memory trace.*
1. #text("for i in range(1, 5):\n    print(i * i)") → memory observes the active instruction and updates the symbolic state.
...
*Memory trace.*
1. #text("n = 7\nfor i in range(1, 6):\n    print(n * i)") → memory observes the active instruction and updates the symbolic state.
...
*Memory trace.*
1. #text("for ligne in range(2):\n    for colonne in range(3):\n        print(ligne, colonne)") → memory observes the active instruction and updates the symbolic state.
...
*Memory trace.*
1. #text("notes = [12, 15, 9]\ntotal = 0\nfor i in range(len(notes)):\n    total += notes[i]\nprint(total / len(notes))") → memory observes the active instruction and updates the symbolic state.
...
*Memory trace.*
1. #text("for i in range(0, 21, 2): print(i)") → memory observes the active instruction and updates the symbolic state.
...
*Memory trace.*
1. #text("i = 0\nwhile i < 5:\n    print(i)\n    i += 1") → memory observes the active instruction and updates the symbolic state.
...
*Memory trace.*
1. #text("age = 0\nwhile age < 1 or age > 120:\n    age = 18\nprint(age)") → memory observes the active instruction and updates the symbolic state.
...
*Memory trace.*
1. #text("n = 5\nwhile n > 0:\n    print(n)\n    n -= 1") → memory observes the active instruction and updates the symbolic state.
...
*Memory trace.*
1. #text("valeur = 1\nwhile valeur < 100:\n    valeur *= 2\nprint(valeur)") → memory observes the active instruction and updates the symbolic state.
...
*Memory trace.*
1. #text("choix = \"q\"\nwhile choix != \"q\":\n    choix = \"q\"\nprint(\"fin\")") → memory observes the active instruction and updates the symbolic state.
...
*Memory trace.*
1. #text("a, b = 48, 18\nwhile b != 0:\n    a, b = b, a % b\nprint(a)") → memory observes the active instruction and updates the symbolic state.
...
*Memory trace.*
1. #text("minute = 0\nwhile minute <= 30:\n    minute += 10\nprint(minute)") → memory observes the active instruction and updates the symbolic state.
